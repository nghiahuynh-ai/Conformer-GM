...
from nemo.collections.asr.parts.submodules.spectr_augment import SpecAugment, SpecCutout, SpecShot
...
class AudioToMelSpectrogramPreprocessor(AudioPreprocessor):
    ...
    def __init__(
        ...
        if (noise_path is not None) and (min_snr > 0) and (max_snr > min_snr):
            try:
                with open(noise_path, 'rb') as f:
                    self.noise_list = np.load(f, allow_pickle=True)
                    self.min_snr = min_snr
                    self.max_snr = max_snr
                    self.augment_list.append(self.add_noise)
                f.close()
            except:
                pass
            
        if (max_semitones > min_semitones):
            self.min_semitones = min_semitones
            self.max_semitones = max_semitones
            self.augment_list.append(self.pitch)
            
        if (n_vol_segments > 0) and (min_rel_vol > 0.0) and (max_rel_vol > min_rel_vol):
            self.n_vol_segments = n_vol_segments
            self.min_rel_vol = min_rel_vol
            self.max_rel_vol = max_rel_vol
            self.augment_list.append(self.rand_volume)
    
    def get_features(self, input_signal, length):
        if len(self.augment_list) > 0:
            augment = self.augment_list[0]
            if len(self.augment_list) > 1:
                augment = np.random.choice(self.augment_list)
    ...
    def add_noise(self, signal):
        ...
        noise = torch.from_numpy(np.array([noise] * batch_size)).to(signal.device)
    ...
    def pitch(self, signal):
        ...
        n_semitones = np.random.randint(self.min_semitones, self.max_semitones)
        tfm = sox.Transformer()
        tfm.pitch(n_semitones=n_semitones)
        
        signal = signal.reshape(batch_size, signal_length)
        signal = torch.from_numpy(signal)
        return signal.to(device)
    
    def rand_volume(self, signal):
        device = signal.device
        signal = signal.cpu().detach().numpy()
        
        batch_size, signal_length = signal.shape
        win_length = int(signal_length / self.n_vol_segments)
        segments = None
        for idx in range(self.n_vol_segments):
            start = idx * win_length
            if idx < self.n_vol_segments - 1:
                segment = signal[:, start:start + win_length]
            else:
                segment = signal[:, start:signal_length]
            scale = np.random.uniform(low=self.min_rel_vol, high=self.max_rel_vol, size=(batch_size, 1))
            segment = scale * segment
            if idx == 0:
                segments = segment
            else:
                segments = np.concatenate((segments, segment), axis=1)
        signal = torch.from_numpy(segments.astype(np.float32))
        return signal.to(device)
...
class SpectrogramAugmentation(NeuralModule):
    ...
    def __init__(
        ...
        if rect_masks > 0:
            ...
            self.augment_type.append('spec_cutout')
    ...
    def forward(self, input_spec, length):
        if len(self.augment_type) == 0:
            return input_spec
        augment = random.choice(self.augment_type)
        if augment == 'spec_augment':
            return self.spec_augment(input_spec=input_spec, length=length)
        elif augment == 'spec_shot':
            return self.spec_shot(input_spec=input_spec)
        else:
            return self.spec_cutout(input_spec=input_spec)
...
class AudioToMelSpectrogramPreprocessorConfig:
    ...
    min_snr: int = 10
    max_snr: int = 20
    min_semitones: float = -5.0
    max_semitones: float = 5.0
    n_vol_segments: int = 3
    min_rel_vol: float = 0.4
    max_rel_vol: float = 10.0
